...
<body>
                            ...
                            <div class="row">
                                <div class="col-xl-12">
                                    <div class="task-list-box" id="landing-task">
                                        ...
                                        <div style="cursor: pointer;" id="task-item-{{ $kuiss->id_kuis }}" data-bs-toggle="modal"  data-bs-target="#modalView-{{ $kuiss->id_kuis }}">
    ...
    @foreach ($kuis as $kuiss )
    <div class="modal fade" id="modalView-{{ $kuiss->id_kuis }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modal-{{ $kuiss->id_kuis }}Label" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-{{ $kuiss->id_kuis }}Label">Kuis {{ $kuiss->judul_kuis}}</h5>
                </div>
                <div class="modal-body">
                    Kuis : <b>{{ $kuiss->judul_kuis}}</b>
                    <br>
                    Bab : <b>{{ $kuiss->nama_mapel}} </b>
                    <br><br><br>
                    Tekan Tombol <strong>Mulai</strong> untuk Mengerjakan kuis
                </div>
                <div class="modal-footer" style="text-align: center">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Kembali</button>
                    @if ($kuiss->id_nilai_kuis == null)
                    <button type="submit" class="btn btn-primary" onclick="window.location.href='/kuis/{{$kuiss->nama_mapel}}/{{$kuiss->id_kuis}}'" data-bs-dismiss="modal">Mulai</button>
                    @else
                    <button type="submit" class="btn btn-primary" onclick="window.location.href='/review/kuis/{{$kuiss->id_kuis}}'" data-bs-dismiss="modal">Review Kuis</button>
                    @endif
    ...
    @endforeach
    {{--@if(session('sedang_mengerjakan_kuis'))
    <script>
        // Redirect otomatis jika mencoba membuka halaman lain
        //window.location.href = "{{ url('/kuis/soal/' . session('id_kuis_sedang_dikerjakan')) }}";
        alert('testtt');
    </script>
    @endif--}}
    ...
    @endif
    @if (session('status_kuis'))
    <script>
      Swal.fire({
          title: "Perhatian !!!",
          text: "{{ session('status_kuis') }}", // Menggunakan blade syntax untuk menampilkan pesan
          icon: "success"
      });
          console.log("Error reset message:", "{{ session('status_kuis') }}");
    </script>
    @endif
