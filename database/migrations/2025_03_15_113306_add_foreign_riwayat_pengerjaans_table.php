<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('riwayat_pengerjaans', function (Blueprint $table) {
            // $table->string('id_siswa',15)->change();
            $table->foreign('id_siswa')->references(columns: 'id_siswa')->on('siswas')->onDelete('cascade');

            // $table->string('id_kuis',15)->change();
            $table->foreign('id_kuis')->references('id_kuis')->on('kuis')->onDelete('cascade');

            // $table->integer('id_soal')->change();
            $table->foreign('id_soal')->references('id_soal')->on('soals')->onDelete('cascade');
        });
    }

    public function down(): void
    {
        Schema::table('riwayat_pengerjaans', function (Blueprint $table) {
            $table->dropForeign(['id_siswa']);
            // $table->string('id_siswa',15)->change();
            
            $table->dropForeign(['id_kuis']);
            // $table->string('id_kuis',15)->change();
            
            $table->dropForeign(['id_soal']);
            // $table->integer('id_soal')->change();
        });
    }
};
